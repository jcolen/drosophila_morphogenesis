...
from ..plot_utils import plot_tensor, plot_scalar
...
def sqh_vel_plot(m, v, t, dt=5, skip=20):
	...
	for i in range(N):
		...
		plot_tensor(ax[i], m[ii], both=t[ii]<0, **mwargs)
	...
	for i, (field, z, z0) in enumerate(fields):
		...
		for j in range(N):
			...
			if n_channels == 1:
				...
				zj[0, 1] *= -1
				zj[1, 0] *= -1
				plot_tensor(ax[i, j], zj * alpha, alpha=alpha, **kwargs)
				plot_tensor(ax[i, j], z0[jj] * np.sign(alpha0), alpha=alpha0, **kwargs)
		...
		if n_channels == 2:
			...
			ax[i, -1].plot(t, znorm.mean(axis=(-1, -2)), color='black')
